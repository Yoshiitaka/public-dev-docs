...
You can find complete, working examples in the [Elixir SDK GitHub repo examples directory](https://github.com/momentohq/client-sdk-elixir/tree/main/examples). 実行可能な完成された例は [Elixir SDK GitHub レポジトリの examples ディレクトリ](https://github.com/momentohq/client-sdk-elixir/tree/main/examples)をご覧下さい。
...
A simple example of doing a set operation. In the CacheClient.set() call, the TTL is optional. 書込み操作を行うシンプルな例です。client.set 呼出しでは、TTL はオプショナルです。もし TTL を渡すと、クライアント接続オブジェクトに設定されたデフォルトの TTL 値が上書きされます。
